...
layout(local_size_x = 1024, local_size_y = 1, local_size_z = 1) in;//先固定为1024个，后面扩展成动态的
...
shared vec4 particleData[1024][1024];//粒子数据
...
void main(void)
{
	...
	particleData[index1.x][index1.y] = vec4(0,0,0,0);
	particleData[index2.x][index2.y] = vec4(0,0,0,0);
	//同步数据
	barrier();
	//约束对这两个粒子起作用
	...
	atomicAdd(inout particleData[index1.x][index1.y]​,deltaP1);
	atomicAdd(inout particleData[index2.x][index2.y]​,deltaP2);
	//这里会出现并行问题，两个约束同时运行，共同影响同一个粒子（x,y），此时在另一个工作组中，imageStore到output_PosDeltaData的同一个位置
	//相同种类的约束会出现冲突
	imageStore(output_PosDeltaData, index1, particleData[index1.x][index1.y]);
	imageStore(output_PosDeltaData, index2, particleData[index2.x][index2.y]);
